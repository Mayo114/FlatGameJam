...
// - Fixed bugs with compound files not being able to write files more than
// 65535 bytes.
...
// - Added Print() to BasicExcelWorksheet to print the worksheet to an output
// stream.
...
// - Remove code for ExtSST because it was causing problems with reading and
// writing to Excel files containing many strings.
...
// - Minor changes to functions in BasicExcel and BasicExcelWorksheet which
// checks for unicode.
...
// - Fixed bug with reading and writing Excel files containing many unicode and
// ANSI strings.
// Version 1.12 (6 June 2006)
// - Fixed bug with reading and writing Excel files containing many unicode and
// ANSI strings.
// Version 1.13 (1 August 2006)
// - Changed BasicExcelCell::Get() so that it will get a stored double as an
// integer or vice versa if necessary.
// - Changed BasicExcelCell::Get() so that it will not cause any errors if a
// string is empty.
// - Changed BasicExcelCell::SetString() and BasicExcelCell::SetWString() so
...
struct LittleEndian {
  ...
  static void Read(const char* buffer, Type& retVal, int pos = 0,
		   int bytes = 0) {
    retVal = Type(0);
  ...
  template <typename Type>
  static void ReadString(const char* buffer, Type* str, int pos = 0,
			 int bytes = 0) {
    for (size_t i = 0; i < bytes; ++i)
  ...
  template <typename Type>
  static void Read(const vector<char>& buffer, Type& retVal, int pos = 0,
		   int bytes = 0) {
    retVal = Type(0);
    if (bytes == 0) bytes = sizeof(Type);
    for (size_t i = 0; i < bytes; ++i) {
  ...
  template <typename Type>
  static void ReadString(const vector<char>& buffer, Type* str, int pos = 0,
			 int bytes = 0) {
    for (size_t i = 0; i < bytes; ++i)
      Read(buffer, str[i], pos + i * sizeof(Type));
  }
  ...
  template <typename Type>
  static void Write(vector<char>& buffer, Type val, int pos = 0,
		    int bytes = 0) {
    if (bytes == 0) bytes = sizeof(Type);
    for (size_t i = 0; i < bytes; ++i) {
      buffer[pos + i] = (unsigned char)val;
  ...
  template <typename Type>
  static void WriteString(vector<char>& buffer, Type* str, int pos = 0,
			  int bytes = 0) {
    for (size_t i = 0; i < bytes; ++i)
      Write(buffer, str[i], pos + i * sizeof(Type));
  }

  static void Read(const char* buffer, wchar_t& retVal, int pos = 0,
		   int bytes = 0) {
    retVal = wchar_t(0);
  ...
  static void ReadString(const char* buffer, wchar_t* str, int pos = 0,
			 int bytes = 0) {
    for (int i = 0; i < bytes; ++i)
  ...
  static void WriteString(char* buffer, wchar_t* str, int pos = 0,
			  int bytes = 0) {
    for (int i = 0; i < bytes; ++i)
      Write(buffer, str[i], pos + i * SIZEOFWCHAR_T);
  }

  static void Read(const vector<char>& buffer, wchar_t& retVal, int pos = 0,
		   int bytes = 0) {
    retVal = wchar_t(0);
    if (bytes == 0) bytes = SIZEOFWCHAR_T;
    for (int i = 0; i < bytes; ++i) {
  ...
  static void ReadString(const vector<char>& buffer, wchar_t* str, int pos = 0,
			 int bytes = 0) {
    for (int i = 0; i < bytes; ++i)
      Read(buffer, str[i], pos + i * SIZEOFWCHAR_T);
  }

  static void Write(vector<char>& buffer, wchar_t val, int pos = 0,
		    int bytes = 0) {
    if (bytes == 0) bytes = SIZEOFWCHAR_T;
    for (int i = 0; i < bytes; ++i) {
      buffer[pos + i] = (unsigned char)val;
  ...
  static void WriteString(vector<char>& buffer, wchar_t* str, int pos = 0,
			  int bytes = 0) {
    for (int i = 0; i < bytes; ++i)
      Write(buffer, str[i], pos + i * SIZEOFWCHAR_T);
  }
...
class CompoundFile {
 ...
 protected:
  ...
  void SplitPath(const wchar_t* path, wchar_t*& parentpath,
  ...
  class Header {
   public:
    ...
    long long fileType_;  // Magic number identifying this as a compound file
			  // system (0x0000)
    ...
    int propertiesStart_;     // Block index of the first block of the property
			      // table (0x0030)
    int uk10_;		      // Unknown constant (0x0034)
    int uk11_;		      // Unknown constant (0x0038)
    int SBATStart_;      // Block index of first big block containing the small
			 // block allocation table (SBAT) (0x003C)
    int SBATCount_;      // Number of big blocks holding the SBAT (0x0040)
    int XBATStart_;      // Block index of the first block in the Extended Block
			 // Allocation Table (XBAT) (0x0044)
    int XBATCount_;      // Number of elements in the Extended Block Allocation
			 // Table (to be added to the BAT) (0x0048)
    int BATArray_[109];  // Array of block indices constituting the Block
			 // Allocation Table (BAT) (0x004C, 0x0050, 0x0054 ...
			 // 0x01FC)
  ...
  class Property {
   public:
    ...
    void Write(char* block);
    void Read(char* block);
    friend bool operator==(const CompoundFile::Property& lhs,
    ...
    wchar_t name_[32];  // A unicode null-terminated uncompressed 16bit string
			// (lblocke the high bytes) containing the name of the
			// property. (0x00, 0x02, 0x04, ... 0x3E)
    short nameSize_;    // Number of characters in the NAME field (0x40)
    unsigned char propertyType_;  // Property type (directory, file, or root)
				  // Byte 1 (directory), 2 (file), or 5 (root
				  // entry) (0x42)
    ...
    int startBlock_;  // Starting block of the file, used as the first block in
		      // the file and the pointer to the next block from the BAT
		      // (0x74)
    int size_;  // Actual size of the file this property points to. (used to
		// truncate the blocks to the real size). (0x78)
  };
  class PropertyTree {
   public:
    PropertyTree();
  ...
  void InsertPropertyTree(PropertyTree* parentTree, Property* property,
			  size_t index);
...
struct CODE {
  enum {
    ...
    CALCCOUNT = 0x000C,  // Maximum number of times the forumlas should be
			 // iteratively calculated
    CALCMODE = 0x000D,  // Calculate formulas manually, automatically, or
			// automatically except for multiple table operations
    PRECISION = 0x000E,  // Whether formulas use the real cell values for
			 // calculation or the values displayed on the screen.
    ...
    ITERATION = 0x0011,  // Whether iterations are allowed while calculating
			 // recursive formulas.
    PROTECT = 0x0012,  // Whether worksheet or a workbook is protected against
		       // modification.
    PASSWORD = 0x0013,  // 16-bit hash value, calculated from the worksheet or
    ...
    WINDOWPROTECT = 0x0019,  // Whether the window configuration of the document
			     // is protected.
    SELECTION = 0x001D,  // Addresses of all selected cell ranges and position
			 // of the active cell for a pane in the current sheet.
    DATEMODE = 0x0022,    // Base date for displaying date values.
    EXTERNNAME = 0x0023,  // Name of an external defined name, name of an add-in
			  // function, a DDE item or an OLE object storage
			  // identifier.
    ...
    PRINTHEADERS = 0x002A,  // Whether row and column headers (the areas with
			    // row numbers and column letters) will be printed.
    PRINTGRIDLINES = 0x002B,  // Whether sheet grid lines will be printed.
    FILEPASS =
	0x002F,     // Information about the read/write password of the file.
    FONT = 0x0031,  // Information about a used font, including character
		    // formatting.
    TABLE =
	0x0036,  // Information about a multiple operation table in the sheet.
    ...
    DCONREF = 0x0051,
    DEFCOLWIDTH = 0x0055,  // Default column width for columns that do not have
    ...
    FILESHARING = 0x005B,  // Information about write protection, for instance
			   // the write protection password.
    WRITEACCESS = 0x005C,  // Name of the user that has saved the file.
    UNCALCED = 0x005E,     // Formulas have not been recalculated before the
			// document was saved.
    SAVERECALC = 0x005F,  //"Recalculate before save" option
    OBJECTPROTECT =
	0x0063,	// Whether objects of the current sheet are protected.
    COLINFO = 0x007D,  // Width for a given range of columns
    GUTS = 0x0080,     // Layout of outline symbols.
    WSBOOL =
	0x0081,  // 16-bit value with boolean options for the current sheet.
    GRIDSET = 0x0082,  // Whether option to print sheet grid lines has ever been
		       // changed.
    ...
    COUNTRY = 0x008C,  // User interface language of the Excel version that has
		       // saved the file, system regional settings at the time
		       // the file was saved.
    ...
    PALETTE = 0x0092,  // Definition of all user-defined colours available for
		       // cell and object formatting.
    SETUP = 0x00A1,    // Page format settings of the current sheet.
    SHRFMLA = 0x00BC,  // Token array of a shared formula.
    MULRK = 0x00BD,    // Cell range containing RK value cells. All cells are
		     // located in the same row.
    MULBLANK = 0x00BE,  // Cell range of empty cells. All cells are located in
			// the same row.
    DBCELL = 0x00D7,  // Relative offsets to calculate stream position of the
		      // first cell record for each row.
    BOOKBOOL = 0x00DA,  // Save values linked from external workbooks records
			// and XCT records?
    SCENPROTECT =
	0x00DD,   // Whether scenarios of the current sheet are protected.
    ...
    EXTSST = 0x00FF,       // Create a hash table with stream offsets to the SST
		      // record to optimise string search operations.
    LABELRANGES = 0x015F,  // Addresses of all row and column label ranges in
			   // the current sheet.
    USESELFS = 0x0160,  // Whether formulas in the workbook can use "natural
			// language formulas".
    DSF = 0x0161,  // Whether file contains an addition BIFF5/BIFF7 workbook
		   // stream.
    SUPBOOK = 0x01AE,  // URL of an external document and a list of sheet names
		       // inside this document.
    CONDFMT = 0x01B0,  // List of cell range addresses for all cells with equal
		       // conditional formatting.
    CF = 0x01B1,  // Condition and the formatting attributes applied to the
		  // cells specified in the CONDFMT record, if the condition is
		  // met
    DVAL =
	0x01B2,  // List header of the data validity table in the current sheet.
    HLINK = 0x01B8,  // One cell address or a cell range where all cells contain
		     // the same hyperlink.
    DV = 0x01BE,  // Data validity settings and a list of cell ranges which
		  // contain these settings.
    DIMENSIONS =
	0x0200,  // Range address of the used area in the current sheet.
    BLANK =
	0x0201,  // Empty cell, contains cell address and formatting information
    ...
    INDEX = 0x020B,    // Range of used rows and stream positions of several
		     // records of the current sheet.
    ARRAY = 0x0221,  // Token array of an array formula
    WINDOW2 =
	0x023E,   // Additional settings for the window of a specific worksheet.
    RK = 0x027E,  // Cell that contains an RK value (encoded integer or floating
		  // point value).
    STYLE = 0x0293,  // Name of a user-defined cell style or specific options
    ...
    SHEETLAYOUT =
	0x0862,  // Colour of the tab below the sheet containing the sheet name.
    SHEETPROTECTION = 0x0867,  // Additional options for sheet protection.
    RANGEPROTECTION = 0x0868  // Information about special protected ranges in a
			      // protected sheet.
  };
};
...
bool IsRKValueAnInteger(int rkValue);  ///< Returns true if the supplied rk
				       ///value contains an integer.
bool IsRKValueADouble(
    int rkValue);  ///< Returns true if the supplied rk value contains a double.
...
bool CanStoreAsRKValue(double value);  ///< Returns true if the supplied double
				       ///can be stored as a rk value.
...
class BasicExcel {
 ...
 public:		     // File functions.
  void New(int sheets = 3);  ///< Create a new Excel workbook with a given
			     ///number of spreadsheets (Minimum 1).
  bool Load(const char* filename);  ///< Load an Excel workbook from a file.
  bool Save();  ///< Save current Excel workbook to opened file.
  bool SaveAs(
      const char* filename);  ///< Save current Excel workbook to a file.

 public:			// Worksheet functions.
  size_t GetTotalWorkSheets();  ///< Total number of Excel worksheets in current
				///Excel workbook.

  BasicExcelWorksheet* GetWorksheet(size_t sheetIndex);  ///< Get a pointer to
							 ///an Excel worksheet
							 ///at the given index.
							 ///Index starts from 0.
							 ///Returns 0 if index
							 ///is invalid.
  BasicExcelWorksheet* GetWorksheet(const char* name);  ///< Get a pointer to an
							///Excel worksheet that
							///has given ANSI name.
							///Returns 0 if there is
							///no Excel worksheet
							///with the given name.
  BasicExcelWorksheet* GetWorksheet(const wchar_t* name);  ///< Get a pointer to
							   ///an Excel worksheet
							   ///that has given
							   ///Unicode name.
							   ///Returns 0 if there
  ...
  BasicExcelWorksheet* AddWorksheet(
      int sheetIndex = -1);  ///< Add a new Excel worksheet to the given index.
			     ///Name given to worksheet is SheetX, where X is a
			     ///number which starts from 1. Index starts from 0.
			     ///Worksheet is added to the last position if
			     ///sheetIndex == -1. Returns a pointer to the
			     ///worksheet if successful, 0 if otherwise.
  BasicExcelWorksheet* AddWorksheet(
      const char* name,
      int sheetIndex = -1);  ///< Add a new Excel worksheet with given ANSI name
			     ///to the given index. Index starts from 0.
			     ///Worksheet is added to the last position if
			     ///sheetIndex == -1. Returns a pointer to the
			     ///worksheet if successful, 0 if otherwise.
  BasicExcelWorksheet* AddWorksheet(
      const wchar_t* name,
      int sheetIndex = -1);  ///< Add a new Excel worksheet with given Unicode
			     ///name to the given index. Index starts from 0.
			     ///Worksheet is added to the last position if
			     ///sheetIndex == -1. Returns a pointer to the
			     ///worksheet if successful, 0 if otherwise.

  bool DeleteWorksheet(size_t sheetIndex);  ///< Delete an Excel worksheet at
					    ///the given index. Index starts
					    ///from 0. Returns true if
					    ///successful, false if otherwise.
  bool DeleteWorksheet(const char* name);   ///< Delete an Excel worksheet that
					    ///has given ANSI name. Returns true
					   ///if successful, false if otherwise.
  bool DeleteWorksheet(const wchar_t* name);  ///< Delete an Excel worksheet
					      ///that has given Unicode name.
					      ///Returns true if successful,
					      ///false if otherwise.

  char* GetAnsiSheetName(size_t sheetIndex);  ///< Get the worksheet name at the
					      ///given index. Index starts from
					      ///0. Returns 0 if name is in
					      ///Unicode format.
  wchar_t* GetUnicodeSheetName(size_t sheetIndex);  ///< Get the worksheet name
						    ///at the given index. Index
						    ///starts from 0. Returns 0
						    ///if name is in Ansi
						    ///format.
  bool GetSheetName(size_t sheetIndex, char* name);  ///< Get the worksheet name
						     ///at the given index.
						     ///Index starts from 0.
						     ///Returns false if name is
						     ///in Unicode format.
  bool GetSheetName(size_t sheetIndex,
		    wchar_t* name);  ///< Get the worksheet name at the given
				     ///index. Index starts from 0. Returns
				     ///false if name is in Ansi format.

  bool RenameWorksheet(size_t sheetIndex,
		       const char* to);  ///< Rename an Excel worksheet at the
					 ///given index to the given ANSI name.
					 ///Index starts from 0. Returns true if
					 ///successful, false if otherwise.
  bool RenameWorksheet(size_t sheetIndex,
		       const wchar_t* to);  ///< Rename an Excel worksheet at
					    ///the given index to the given
					    ///Unicode name. Index starts from
					    ///0. Returns true if successful,
					    ///false if otherwise.
  bool RenameWorksheet(const char* from,
		       const char* to);  ///< Rename an Excel worksheet that has
					 ///given ANSI name to another ANSI
					 ///name. Returns true if successful,
					 ///false if otherwise.
  bool RenameWorksheet(const wchar_t* from,
		       const wchar_t* to);  ///< Rename an Excel worksheet that
					    ///has given Unicode name to another
					    ///Unicode name. Returns true if
					    ///successful, false if otherwise.
 ...
 private:			 // Internal functions
  void UpdateYExcelWorksheet();  ///< Update yesheets_ using information from
				 ///worksheets_.
  void
  UpdateWorksheets();  ///< Update worksheets_ using information from yesheets_.
...
class BasicExcelWorksheet {
 ...
 public:		     // Worksheet functions
  char* GetAnsiSheetName();  ///< Get the current worksheet name. Returns 0 if
			     ///name is in Unicode format.
  wchar_t* GetUnicodeSheetName();  ///< Get the current worksheet name. Returns
				   ///0 if name is in Ansi format.
  bool GetSheetName(char* name);   ///< Get the current worksheet name. Returns
				   ///false if name is in Unicode format.
  bool GetSheetName(wchar_t* name);  ///< Get the current worksheet name.
				     ///Returns false if name is in Ansi format.
  bool Rename(const char* to);  ///< Rename current Excel worksheet to another
				///ANSI name. Returns true if successful, false
				///if otherwise.
  bool Rename(const wchar_t* to);  ///< Rename current Excel worksheet to
				   ///another Unicode name. Returns true if
				   ///successful, false if otherwise.
  void Print(ostream& os, char delimiter = ',',
	     char textQualifier = '\0');  ///< Print entire worksheet to an
					  ///output stream, separating each
					  ///column with the defined delimiter
					  ///and enclosing text using the
					  ///defined textQualifier. Leave out
					  ///the textQualifier argument if do
					  ///not wish to have any text
					  ///qualifiers.

 public:		  // Cell functions
  size_t GetTotalRows();  ///< Total number of rows in current Excel worksheet.
  size_t
  GetTotalCols();  ///< Total number of columns in current Excel worksheet.

  BasicExcelCell* Cell(size_t row, size_t col);  ///< Return a pointer to an
						 ///Excel cell. row and col
						 ///starts from 0. Returns 0 if
						 ///row exceeds 65535 or col
						 ///exceeds 255.
  bool EraseCell(size_t row, size_t col);  ///< Erase content of a cell. row and
					   ///col starts from 0. Returns true if
					   ///successful, false if row or col
					   ///exceeds range.

 private:	      // Internal functions
  void UpdateCells();  ///< Update cells using information from
		       ///BasicExcel.worksheets_.
...
class BasicExcelCell {
 ...
 public:
  ...
  int Type() const;  ///< Get type of value stored in current Excel cell.
		     ///Returns one of the above enums.

  bool Get(int& val) const;     ///< Get an integer value. Returns false if cell
				///does not contain an integer or a double.
  bool Get(double& val) const;  ///< Get a double value. Returns false if cell
				///does not contain a double or an integer.
  bool Get(char* str) const;    ///< Get an ANSI string. Returns false if cell
				///does not contain an ANSI string.
  bool Get(wchar_t* str) const;    ///< Get an Unicode string. Returns false if
				   ///cell does not contain an Unicode string.
  size_t GetStringLength() const;  ///< Return length of ANSI or Unicode string
				   ///(excluding null character).

  int GetInteger() const;  ///< Get an integer value. Returns 0 if cell does not
			   ///contain an integer.
  double GetDouble() const;  ///< Get a double value. Returns 0.0 if cell does
			     ///not contain a double.
  const char* GetString() const;      ///< Get an ANSI string. Returns 0 if cell
				      ///does not contain an ANSI string.
  const wchar_t* GetWString() const;  ///< Get an Unicode string. Returns 0 if
				      ///cell does not contain an Unicode
				      ///string.

  friend ostream& operator<<(ostream& os,
			     const BasicExcelCell& cell);  ///< Print cell to
							   ///output stream.
							   ///Print a null
							   ///character if cell
							   ///is undefined.
  ...
  void Set(const char*
	       str);  ///< Set content of current Excel cell to an ANSI string.
  void Set(const wchar_t* str);  ///< Set content of current Excel cell to an
				 ///Unicode string.

  void SetInteger(
      int val);  ///< Set content of current Excel cell to an integer.
  void SetDouble(
      double val);  ///< Set content of current Excel cell to a double.
  void SetString(const char* str);  ///< Set content of current Excel cell to an
				    ///ANSI string.
  void SetWString(const wchar_t* str);  ///< Set content of current Excel cell
					///to an Unicode string.

  void EraseContents();  ///< Erase the content of current Excel cell. Set type
			 ///to UNDEFINED.

 private:
  int type_;  ///< Type of value stored in current Excel cell. Contains one of
	      ///the above enums.
  int ival_;  ///< Integer value stored in current Excel cell.
  double dval_;		  ///< Double value stored in current Excel cell.
  vector<char> str_;      ///< ANSI string stored in current Excel cell. Include
			  ///null character.
  vector<wchar_t> wstr_;  ///< Unicode string stored in current Excel cell.
