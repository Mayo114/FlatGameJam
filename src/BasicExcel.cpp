...
// PROMISE: Return true if file is successfully created and opened, false if
// otherwise.
{
  // Create new file
...
// PURPOSE: Move a block of data in the opened file from an index position to
// another index position.
...
void CompoundFile::Header::Initialize() {
  bigBlockSize_ =
      (size_t)pow(2.0, log2BigBlockSize_);  // Calculate each big block size.
  smallBlockSize_ = (size_t)pow(
      2.0, log2SmallBlockSize_);  // Calculate each small block size.
}
/********************************** End of Class Header
 * **************************************/
...
// PURPOSE: Reset BAT indices, SBAT indices, properties and properties tree
// information.
// PROMISE: Return true if file is successfully closed, false if otherwise.
{
  blocksIndices_.clear();
...
// PROMISE: Directory will not be removed if it has subdirectories or files
// under it.
{
...
int CompoundFile::DirectoryList(vector<vector<wchar_t> >& list,
				const wchar_t* path) {
  previousDirectories_.push_back(currentDirectory_);
  if (path != 0) {
...
// PROMISE: The file's original data will be replaced by the new data.
{
  ...
  if (property->self_->size_ >= 4096) {
    if (size >= 4096)
	  ...
	  WriteData(data, size, property->self_->startBlock_, true);
    else {
      property->self_->startBlock_ =
	  WriteData(0, 0, property->self_->startBlock_, true);
      property->self_->startBlock_ =
	  WriteData(data, size, property->self_->startBlock_, false);
    }
  } else {
    if (size < 4096)
      property->self_->startBlock_ =
      ...
      property->self_->startBlock_ =
	  WriteData(0, 0, property->self_->startBlock_, false);
      property->self_->startBlock_ =
	  WriteData(data, size, property->self_->startBlock_, true);
    }
  }
...
int CompoundFile::WriteFile(const wchar_t* path, const vector<char>& data,
			    size_t size)
// PURPOSE: Write data to a file in the compound file.
// PROMISE: The file's original data will be replaced by the new data.
{
...
/*************ANSI char compound file, directory and file
 * functions******************/
...
// PURPOSE: Increase block location references in header, BAT indices and
// properties,
// PURPOSE: which will be affected by the insertion of new indices contained in
// indices.
...
// PROMISE: Changes will not be written to compound file.
{
  ...
  // Change BAT Array references
  {
    for (size_t i = 0; i < 109 && header_.BATArray_[i] != -1; ++i) {
      size_t count = 0;
      for (size_t j = 0; j < maxIndices; ++j) {
	if (header_.BATArray_[i] >= indices[j] && header_.BATArray_[i] != -1)
	  ++count;
      }
  ...
  if (header_.XBATCount_) {
    size_t count = 0;
    for (size_t j = 0; j < maxIndices; ++j) {
      if (header_.XBATStart_ >= indices[j] && header_.XBATStart_ != -2) ++count;
    }
  ...
  if (header_.SBATCount_) {
    size_t count = 0;
    for (size_t j = 0; j < maxIndices; ++j) {
      if (header_.SBATStart_ >= indices[j] && header_.SBATStart_ != -2) ++count;
    }
  ...
  size_t maxBATindices = blocksIndices_.size();
  {
    for (size_t i = 0; i < maxBATindices && blocksIndices_[i] != -1; ++i) {
      size_t count = 0;
      for (size_t j = 0; j < maxIndices; ++j) {
	if (blocksIndices_[i] > indices[j] && blocksIndices_[i] != -2 &&
	    blocksIndices_[i] != -3)
	  ++count;
      }
...
// PURPOSE: Decrease block location references in header, BAT indices and
// properties,
// PURPOSE: which will be affected by the deletion of indices contained in
// indices.
// PROMISE: BAT indices pointing to a deleted index will be redirected to point
// to
...
// PROMISE: Changes will not be written to compound file.
{
  ...
  }
  {
    for (size_t i = 1; i < maxProperties; ++i) {
      if (properties_[i]->size_ >= 4096) {
	// Change individual properties start block if their size is more than
	// 4096
	size_t count = 0;
	for (size_t j = 0; j < maxIndices; ++j) {
	  if (properties_[i]->startBlock_ > indices[j] &&
...
void CompoundFile::SplitPath(const wchar_t* path, wchar_t*& parentpath,
			     wchar_t*& propertyname)
// PURPOSE: Get a path's parent path and its name.
// EXPLAIN: E.g. path = "\\Abc\\def\\ghi => parentpath = "\\Abc\\def",
// propertyname = "ghi".
...
// PURPOSE: Load all block allocation table information for compound file.
{
  // Read BAT indices
  {
	...
	LittleEndian::Read(&*(block_.begin()), blocksIndices_[j + i * 128],
			   j * 4, 4);
      }
    }
  }
  ...
  // Read XBAT indices
  {
    for (size_t i = 0; i < header_.XBATCount_; ++i) {
      ...
      for (size_t j = 0; j < 128; ++j) {
	LittleEndian::Read(&*(block_.begin()),
			   blocksIndices_[j + ((i + 109) * 128)], j * 4, 4);
      }
    }
  }
  ...
  // Read SBAT indices
  {
    for (size_t i = 0; i < header_.SBATCount_; ++i) {
      ...
      for (size_t j = 0; j < 128; ++j) {
	LittleEndian::Read(&*(block_.begin()), sblocksIndices_[j + i * 128],
			   j * 4, 4);
      }
    }
  }
...
// PURPOSE: Save all block allocation table information for compound file.
{
  // Write BAT indices
  {
    for (size_t i = 0; i < header_.BATCount_; ++i) {
      for (size_t j = 0; j < 128; ++j) {
	LittleEndian::Write(&*(block_.begin()), blocksIndices_[j + i * 128],
			    j * 4, 4);
      }
  ...
  // Write XBAT indices
  {
    for (size_t i = 0; i < header_.XBATCount_; ++i) {
      for (size_t j = 0; j < 128; ++j) {
	LittleEndian::Write(&*(block_.begin()),
			    blocksIndices_[j + ((i + 109) * 128)], j * 4, 4);
      }
  ...
  // Write SBAT indices
  {
    for (size_t i = 0; i < header_.SBATCount_; ++i) {
      for (size_t j = 0; j < 128; ++j) {
	LittleEndian::Write(&*(block_.begin()), sblocksIndices_[j + i * 128],
			    j * 4, 4);
      }
...
// PURPOSE: Gets the total size occupied by a property, starting from
// startIndex.
// EXPLAIN: isBig is true if property uses big blocks, false if it uses small
// blocks.
...
// PROMISE: number of blocks occupied multiply by the block size.
{
  ...
  if (isBig) {
    ...
    size_t smallBlocksPerBigBlock =
	header_.bigBlockSize_ / header_.smallBlockSize_;
    ...
    size_t maxIndices = indices.size();
    for (size_t i = 0; i < maxIndices; ++i) {
      size_t start = (indices[i] - minBlock * smallBlocksPerBigBlock) *
		     header_.smallBlockSize_;
      copy(buffer + start, buffer + start + header_.smallBlockSize_,
...
size_t CompoundFile::WriteData(const char* data, size_t size, int startIndex,
			       bool isBig)
...
// PROMISE: Returns the startIndex of new data for the property.
{
  if (isBig) {
    ...
    // Readjust indices and remove blocks if new data size is smaller than
    // original
	...
	startIndex = -2;
      ...
      copy(indices.begin() + maxNewBlocks, indices.end(),
	...
	file_.Write(indices[curIndex] + 1,
    ...
    if (remainingFullBlocks != 0) {
      // Require extra blocks to write data (i.e. new data is larger than
      // original data
      do {
	size_t newIndex =
	    GetFreeBlockIndex(true);  // Get new free block to write data
	if (startIndex == -2)
	...
	newIndex = indices[curIndex];
      ...
      copy(data + curIndex * header_.bigBlockSize_,
	   data + curIndex * header_.bigBlockSize_ + extraSize,
	   tempdata.begin());
	...
	size_t newIndex =
	    GetFreeBlockIndex(false);  // Get new free block to write data
	if (startIndex == -2)
	  startIndex = newIndex;  // Get start index
	else
    ...
    if (extraSize != 0) {
      copy(data + fullBlocks * header_.smallBlockSize_,
	   ...
	   smallBlocksData.begin() +
	       indices[fullBlocks] * header_.smallBlockSize_);
    }
...
void CompoundFile::GetBlockIndices(size_t startIndex, vector<size_t>& indices,
				   bool isBig)
// PURPOSE: Get the indices of blocks where data are stored, starting from
// startIndex.
// EXPLAIN: isBig is true if property uses big blocks, false if it uses small
// blocks.
{
...
// PROMISE: It only adjust BAT arrays and indices or SBAT arrays and indices so
// that
// PROMISE: it gives the index of a new block where data can be inserted.
{
  ...
  if (isBig) {
    ...
    if (index == sblocksIndices_.size()) {
      ...
      index =
	  distance(sblocksIndices_.begin(),
		   find(sblocksIndices_.begin(), sblocksIndices_.end(), -1));
    }
...
// blocks.
{
  ...
  if (isBig) {
    size_t BATindex =
	distance(&header_.BATArray_[0],
		 find(header_.BATArray_, header_.BATArray_ + 109, -1));
    if (BATindex < 109) {
...
// blocks.
{
  if (isBig) {
    ...
    size_t maxIndices = indices.size();
    {
      for (size_t i = 0; i < maxIndices; ++i) ++indices[i];
    ...
    while (distance(find(blocksIndices_.begin(), blocksIndices_.end(), -1),
		    blocksIndices_.end()) >= 128) {
      blocksIndices_.resize(blocksIndices_.size() - 128);
      if (header_.XBATCount_ != 0) {
	// Shrink XBAT first
	--header_.XBATCount_;
	indicesToRemove.push_back(
	    header_.XBATStart_ + header_.XBATCount_ +
    ...
    char* newdata =
	new char[properties_[0]->size_ - maxIndices * header_.smallBlockSize_];
    {
    ...
    properties_[0]->startBlock_ = WriteData(
	newdata, properties_[0]->size_ - maxIndices * header_.smallBlockSize_,
	properties_[0]->startBlock_, true);
	...
	for (size_t j = 0; j < maxSBATindices; ++j) {
	  ...
	  if (sblocksIndices_[j] > indices[i] && sblocksIndices_[j] != -1 &&
	      sblocksIndices_[j] != -2)
	    --sblocksIndices_[j];
	}
      }
...
// PURPOSE: Load properties information for compound file.
{
  ...
  InsertPropertyTree(propertyTrees_, properties_[properties_[0]->childProp_],
...
int CompoundFile::MakeProperty(const wchar_t* path,
			       CompoundFile::Property* property)
...
CompoundFile::PropertyTree* CompoundFile::FindProperty(
    CompoundFile::PropertyTree* parentTree, wchar_t* name)
// PURPOSE: Find property in the compound file, given the parent property tree
// and its name.
// PROMISE: Returns a pointer to the property tree of the property if property
// PROMISE: is present, 0 if otherwise.
{
...
void CompoundFile::InsertPropertyTree(CompoundFile::PropertyTree* parentTree,
				      CompoundFile::Property* property,
				      size_t index)
// PURPOSE: Insert a property and all its siblings and children into the
// property tree.
// REQUIRE: If the property is a new property and its index is already occupied
// by
// REQUIRE: another property, the calling function has to call
// IncreasePropertyReferences()
// REQUIRE: first before calling this function.
// EXPLAIN: This function is used by LoadProperty() to initialize the property
// trees
...
// PROMISE: The property will be added as the parent tree's child and the
// parent's
// PROMISE: child property and all the its children previous property and next
// property
// PROMISE: will be readjusted to accomodate the next property.
{
  ...
  if (property->previousProp_ != -1) {
    InsertPropertyTree(parentTree, properties_[property->previousProp_],
		       property->previousProp_);
  }

  if (property->nextProp_ != -1) {
    InsertPropertyTree(parentTree, properties_[property->nextProp_],
		       property->nextProp_);
  }

  if (property->childProp_ != -1) {
    InsertPropertyTree(tree, properties_[property->childProp_],
		       property->childProp_);
  }
...
// PROMISE: The tree's parent's child property and all the its children previous
// property
// PROMISE: and next property will be readjusted to accomodate the deleted
// property.
{
...
void CompoundFile::UpdateChildrenIndices(
    CompoundFile::PropertyTree* parentTree) {
  ...
  if (maxChildren != 0) {
    ...
    if (nextChild < maxChildren) {
      ...
      for (++curChild, ++nextChild; nextChild < maxChildren;
	   ++curChild, ++nextChild) {
...
void CompoundFile::IncreasePropertyReferences(
    CompoundFile::PropertyTree* parentTree, size_t index)
// PURPOSE: Increase all property references (previous property, next property
// PURPOSE: and child property) which will be affected by the insertion of the
// new index.
// EXPLAIN: The recursive method of going through each property tree is used
// instead of
// EXPLAIN: using the iterative method of going through each property in
// properties_ is
...
void CompoundFile::DecreasePropertyReferences(
    CompoundFile::PropertyTree* parentTree, size_t index)
// PURPOSE: Decrease all property references (previous property, next property
// PURPOSE: and child property) which will be affected by the deletion of the
// index.
// EXPLAIN: The recursive method of going through each property tree is used
// instead of
// EXPLAIN: using the iterative method of going through each property in
...
size_t Record::Read(const char* data) {
  ...
  while (code == CODE::CONTINUE) {
    ...
    data_.insert(data_.end(), data + recordSize_ + 4,
		 data + recordSize_ + 4 + size);
    dataSize_ += size;
    recordSize_ += 4 + size;
...
size_t Record::Write(char* data) {
  ...
  if (continueIndices_.empty()) {
    ...
    while (size > 8224) {
      ...
      npos += 2;
      size -= 8224;
      copy(data_.begin() + i * 8224, data_.begin() + (i + 1) * 8224,
	   data + npos);
      npos += 8224;
    ...
    npos += 2;
    copy(data_.begin() + i * 8224, data_.begin() + i * 8224 + size,
	 data + npos);
    npos += size;
  } else {
    ...
    for (c = 1; c < maxContinue; ++c) {
      ...
      copy(data_.begin() + continueIndices_[c - 1],
	   data_.begin() + continueIndices_[c], data + npos);
      npos += size;
    }
    LittleEndian::Write(data, 0x3C, npos, 2);  // Write CONTINUE code.
    npos += 2;
    ...
    LittleEndian::Write(data, size, npos, 2);
    npos += 2;
    copy(data_.begin() + continueIndices_[c - 1], data_.end(), data + npos);
    npos += size;
  }
  return npos;
}
...
Workbook::Window1::Window1()
    : Record(),
      ...
      height_(0x2454),
      options_(0x38),
      activeWorksheetIndex_(0),
      firstVisibleTabIndex_(0),
...
Workbook::Style::Style()
    : Record(), XFRecordIndex_(0x8000), identifier_(0), level_(0xFF) {
  code_ = CODE::STYLE;
  dataSize_ = 2;
  recordSize_ = 6;
}
...
Workbook::BoundSheet::BoundSheet()
    : Record(), BOFpos_(0x0000), visibility_(0), type_(0) {
...
/************************************************************************************************************/
Workbook::SharedStringTable::SharedStringTable()
    : Record(), stringsTotal_(0), uniqueStringsTotal_(0) {
  code_ = CODE::SST;
  dataSize_ = 8;
  recordSize_ = 12;
}
size_t Workbook::SharedStringTable::Read(const char* data) {
  ...
  if (continueIndices_.empty()) {
    ...
    for (size_t i = 0, c = 0; i < uniqueStringsTotal_; ++i) {
      ...
      if (c >= maxContinue ||
	  npos + stringSize * multiplier + 3 <= continueIndices_[c]) {
	...
	++c;
	if (size > 0) {
	  size /= multiplier;  // Number of characters available for string in
			       // current record.
	  bytesRead += strings_[i].ContinueRead(
	      &*(data_.begin()) + npos + bytesRead, size);
	  stringSize -= size;
	  size = 0;
	}
	while (c < maxContinue && npos + stringSize + 1 > continueIndices_[c]) {
	  size_t dataSize =
	      (continueIndices_[c] - continueIndices_[c - 1] - 1) / multiplier;
	  bytesRead += strings_[i].ContinueRead(
	      &*(data_.begin()) + npos + bytesRead, dataSize);
	  stringSize -= dataSize + 1;
	  ++c;
	};
	if (stringSize > 0) {
	  bytesRead += strings_[i].ContinueRead(
	      &*(data_.begin()) + npos + bytesRead, stringSize);
	}
	npos += bytesRead;
      }
    }
...
size_t Workbook::SharedStringTable::DataSize() {
  ...
  for (size_t i = 0; i < uniqueStringsTotal_; ++i) {
    ...
    if (dataSize_ + stringSize + 3 <= curMax) {
      ...
      // If have >= 12 bytes (2 for size, 1 for unicode and >=9 for data, can
      // split string
	...
	size_t additionalContinueRecords =
	    unicode ? stringSize / 8222
		    : stringSize / 8223;  // 8222 or 8223 because the first byte
					  // is for unicode identifier
	...
	if (dataSize_ + stringSize + 3 < curMax) {
	  ...
	  // If have >= 12 bytes (2 for size, 1 for unicode and >=9 for data,
	  // can split string
	  // otherwise, end record and start continue record.
	  if (curMax - dataSize_ >= 12) {
	    if (unicode && !((curMax - dataSize_) % 2))
	      --curMax;  // Make sure space reserved for unicode strings is
			 // even.
	    continueIndices_.push_back(curMax);
	    stringSize -= (curMax - dataSize_ - 3);
	    dataSize_ = curMax;
	    ...
	    size_t additionalContinueRecords =
		unicode ? stringSize / 8222
			: stringSize / 8223;  // 8222 or 8223 because the first
					      // byte is for unicode identifier
	    for (size_t j = 0; j < additionalContinueRecords; ++j) {
	      if (unicode) {
		--curMax;
		continueIndices_.push_back(curMax);
...
size_t Workbook::ExtSST::RecordSize() {
  return (recordSize_ =
	      DataSize() + (int)((2 + streamPos_.size() * 8) / 8224) * 4) +
	 4;
}
/************************************************************************************************************/
...
Worksheet::Index::Index()
    : Record(),
      unused1_(0),
      firstUsedRowIndex_(0),
      firstUnusedRowIndex_(0),
      unused2_(0) {
  code_ = CODE::INDEX;
  dataSize_ = 16;
  recordSize_ = 20;
...
Worksheet::Dimensions::Dimensions()
    : Record(),
      firstUsedRowIndex_(0),
      lastUsedRowIndexPlusOne_(0),
      firstUsedColIndex_(0),
      lastUsedColIndexPlusOne_(0),
      unused_(0) {
...
/************************************************************************************************************/
Worksheet::CellTable::RowBlock::CellBlock::Blank::Blank()
    : Record(), rowIndex_(0), colIndex_(0), XFRecordIndex_(0) {
  code_ = CODE::BLANK;
  dataSize_ = 6;
  recordSize_ = 10;
}
size_t Worksheet::CellTable::RowBlock::CellBlock::Blank::Read(
    const char* data) {
  Record::Read(data);
...
Worksheet::CellTable::RowBlock::CellBlock::BoolErr::BoolErr()
    : Record(),
      rowIndex_(0),
      colIndex_(0),
      XFRecordIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::BoolErr::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  LittleEndian::Read(data_, colIndex_, 2, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::LabelSST::LabelSST()
    : Record(),
      rowIndex_(0),
      colIndex_(0),
      XFRecordIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::LabelSST::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  LittleEndian::Read(data_, colIndex_, 2, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::MulBlank::MulBlank()
    : Record(), rowIndex_(0), firstColIndex_(0), lastColIndex_(0) {
  code_ = CODE::MULBLANK;
  dataSize_ = 10;
  recordSize_ = 14;
}
size_t Worksheet::CellTable::RowBlock::CellBlock::MulBlank::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
...
void Worksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK::Read(
    const char* data) {
...
Worksheet::CellTable::RowBlock::CellBlock::MulRK::MulRK()
    : Record(), rowIndex_(0), firstColIndex_(0), lastColIndex_(0) {
  code_ = CODE::MULRK;
  dataSize_ = 10;
  recordSize_ = 14;
}
size_t Worksheet::CellTable::RowBlock::CellBlock::MulRK::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  LittleEndian::Read(data_, firstColIndex_, 2, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::Number::Number()
    : Record(), rowIndex_(0), colIndex_(0), XFRecordIndex_(0), value_(0) {
  code_ = CODE::NUMBER;
  dataSize_ = 14;
  recordSize_ = 18;
}
size_t Worksheet::CellTable::RowBlock::CellBlock::Number::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  LittleEndian::Read(data_, colIndex_, 2, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::RK::RK()
    : Record(), rowIndex_(0), colIndex_(0), XFRecordIndex_(0), value_(0) {
  code_ = CODE::RK;
  dataSize_ = 10;
  recordSize_ = 14;
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  LittleEndian::Read(data_, colIndex_, 2, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::Array()
    : Record(),
      firstRowIndex_(0),
      lastRowIndex_(0),
      firstColIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::Read(
    const char* data) {
  Record::Read(data);
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::Write(
    char* data) {
  data_.resize(DataSize());
...
Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::ShrFmla()
    : Record(),
      firstRowIndex_(0),
      lastRowIndex_(0),
      firstColIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, firstRowIndex_, 0, 2);
  LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Write(
    char* data) {
  data_.resize(DataSize());
  LittleEndian::Write(data_, firstRowIndex_, 0, 2);
  LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
size_t
Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::RecordSize() {
  size_t dataSize = DataSize();
  return (recordSize_ = dataSize + 4 * (dataSize / 8224 + 1));
}

Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::ShrFmla1()
    : Record(),
      firstRowIndex_(0),
      lastRowIndex_(0),
      firstColIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, firstRowIndex_, 0, 2);
  LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Write(
    char* data) {
  data_.resize(DataSize());
  LittleEndian::Write(data_, firstRowIndex_, 0, 2);
  LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
size_t
Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::DataSize() {
  return (dataSize_ = 8 + formula_.size());
}
size_t
Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::RecordSize() {
  size_t dataSize = DataSize();
  return (recordSize_ = dataSize + 4 * (dataSize / 8224 + 1));
}

Worksheet::CellTable::RowBlock::CellBlock::Formula::Table::Table()
    : Record(),
      firstRowIndex_(0),
      lastRowIndex_(0),
      firstColIndex_(0),
      lastColIndex_(0),
      options_(0),
      inputCellRowIndex_(0),
      inputCellColIndex_(0),
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Table::Read(
    const char* data) {
  Record::Read(data);
  LittleEndian::Read(data_, firstRowIndex_, 0, 2);
  LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Table::Write(
    char* data) {
  data_.resize(DataSize());
  LittleEndian::Write(data_, firstRowIndex_, 0, 2);
  LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::String::Read(
    const char* data) {
  Record::Read(data);
  string_.clear();
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::String::Write(
    char* data) {
  data_.resize(DataSize());
...
size_t
Worksheet::CellTable::RowBlock::CellBlock::Formula::String::RecordSize() {
  size_t dataSize = DataSize();
  return (recordSize_ = dataSize + 4 * (dataSize / 8224 + 1));
}
/************************************************************************************************************/
...
/************************************************************************************************************/
Worksheet::CellTable::RowBlock::DBCell::DBCell()
    : Record(), firstRowOffset_(0) {
  code_ = CODE::DBCELL;
  dataSize_ = 4;
  recordSize_ = 8;
}
...
/************************************************************************************************************/
Worksheet::Window2::Window2()
    : Record(),
      options_(1718),
      firstVisibleRowIndex_(0),
      firstVisibleColIndex_(0),
      gridLineColourIndex_(64),
      unused1_(0),
      magnificationFactorPageBreakPreview_(0),
      magnificationFactorNormalView_(0),
      unused2_(0) {
  code_ = CODE::WINDOW2;
  dataSize_ = 18;
  recordSize_ = 22;
...
bool IsRKValueAnInteger(int rkValue) { return (rkValue & 2); }

// Returns true if the supplied rk value contains a double.
bool IsRKValueADouble(int rkValue) { return !(rkValue & 2); }
...
BasicExcel::BasicExcel(const char* filename) { Load(filename); }
...
size_t BasicExcel::GetTotalWorkSheets() { return worksheets_.size(); }
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
BasicExcelWorksheet* BasicExcel::AddWorksheet(const char* name,
					      int sheetIndex) {
  size_t maxWorksheets = yesheets_.size();
  for (size_t i = 0; i < maxWorksheets; ++i) {
    if (workbook_.boundSheets_[i].name_.unicode_ & 1) continue;
  ...
  if (sheetIndex == -1) {
    ...
    boundSheet = &*(workbook_.boundSheets_.insert(
	workbook_.boundSheets_.begin() + sheetIndex, Workbook::BoundSheet()));
    worksheet =
	&*(worksheets_.insert(worksheets_.begin() + sheetIndex, Worksheet()));
    yesheet = &*(yesheets_.insert(yesheets_.begin() + sheetIndex,
				  BasicExcelWorksheet(this, sheetIndex)));
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
BasicExcelWorksheet* BasicExcel::AddWorksheet(const wchar_t* name,
					      int sheetIndex) {
  size_t maxWorksheets = yesheets_.size();
  for (size_t i = 0; i < maxWorksheets; ++i) {
    if (!(workbook_.boundSheets_[i].name_.unicode_ & 1)) continue;
...
// Rename an Excel worksheet that has given Unicode name to another Unicode
// name.
// Returns true if successful, false if otherwise.
...
void BasicExcel::AdjustDBCellPositions() {
  ...
  for (size_t i = 0; i < maxSheets; ++i) {
	...
	for (size_t k = 0; k < maxRows; ++k) {
	  offset +=
	      worksheets_[i].cellTable_.rowBlocks_[j].rows_[k].RecordSize();
	  firstRowOffset +=
	      worksheets_[i].cellTable_.rowBlocks_[j].rows_[k].RecordSize();
	}
      }
      ...
      size_t maxCellBlocks =
	  worksheets_[i].cellTable_.rowBlocks_[j].cellBlocks_.size();
      {
	for (size_t k = 0; k < maxCellBlocks; ++k) {
	  offset += worksheets_[i]
			.cellTable_.rowBlocks_[j]
			.cellBlocks_[k]
			.RecordSize();
	  firstRowOffset += worksheets_[i]
				.cellTable_.rowBlocks_[j]
				.cellBlocks_[k]
				.RecordSize();
	}
      ...
      worksheets_[i].cellTable_.rowBlocks_[j].dbcell_.firstRowOffset_ =
	  firstRowOffset;
	...
	for (size_t k = 0; k < maxRows; ++k) {
	  for (; l < maxCellBlocks; ++l) {
		...
		worksheets_[i]
		    ...
		    .cellBlocks_[l]
		    .RowIndex()) {
	      worksheets_[i].cellTable_.rowBlocks_[j].dbcell_.offsets_[k] =
		  cellOffset;
	      break;
	    }
	    cellOffset += worksheets_[i]
			      .cellTable_.rowBlocks_[j]
			      .cellBlocks_[l]
			      .RecordSize();
	  }
...
void BasicExcel::AdjustExtSSTPositions() {
  ...
  size_t maxPortions =
      workbook_.sst_.uniqueStringsTotal_ / workbook_.extSST_.stringsTotal_ +
      (workbook_.sst_.uniqueStringsTotal_ % workbook_.extSST_.stringsTotal_
	   ? 1
	   : 0);
  ...
  for (size_t i = 0; i < maxPortions; ++i) {
    ...
    for (size_t j = 0; j < workbook_.extSST_.stringsTotal_; ++j) {
      if (i * workbook_.extSST_.stringsTotal_ + j >=
	  workbook_.sst_.strings_.size())
	break;
      size_t stringSize =
	  workbook_.sst_.strings_[i * workbook_.extSST_.stringsTotal_ + j]
	      .StringSize();
	...
	if (8224 - relativeOffset >= 12) {
	  ...
	  if (relativeOffset + stringSize + 3 < 8224) {
	    ...
	    if (8224 - relativeOffset >= 12) {
	      ...
	      size_t additionalContinueRecords =
		  stringSize /
		  8223;  // 8223 because the first byte is for unicode
	      for (size_t k = 0; k < additionalContinueRecords; ++k) {
		stringSize -= 8223;
	      }
	      relativeOffset += stringSize + 1;
...
void BasicExcel::UpdateWorksheets() {
  ...
  for (size_t s = 0; s < maxWorksheets; ++s) {
    ...
    // Modify Index
    worksheets_[s].index_.firstUsedRowIndex_ =
	100000;  // Use 100000 to indicate that firstUsedRowIndex is not set yet
		 // since maximum allowed rows in Excel is 65535.
    worksheets_[s].index_.firstUnusedRowIndex_ = maxRows;

    // Modify Dimensions
    worksheets_[s].dimensions_.firstUsedRowIndex_ =
	100000;  // Use 100000 to indicate that firstUsedRowIndex is not set yet
		 // since maximum allowed rows in Excel is 65535.
    worksheets_[s].dimensions_.firstUsedColIndex_ =
	1000;  // Use 1000 to indicate that firstUsedColIndex is not set yet
	       // since maximum allowed columns in Excel is 255.
    ...
    vector<Worksheet::CellTable::RowBlock>& rRowBlocks =
	worksheets_[s].cellTable_.rowBlocks_;
	...
	if (cellType !=
	    BasicExcelCell::UNDEFINED)  // Current cell contains some data
	{
	  if (worksheets_[s].index_.firstUsedRowIndex_ == 100000) {
	    ...
	    size_t nm = int(worksheets_[s].index_.firstUnusedRowIndex_ -
			    worksheets_[s].index_.firstUsedRowIndex_ - 1) /
			    32 +
			1;
	  ...
	  if (newRow) {
	    ...
	    Worksheet::CellTable::RowBlock& rRowBlock =
		rRowBlocks[curRowBlock - 1];
	  ...
	  switch (cellType) {
		...
		for (size_t i = 0; c < cl; ++c, ++i) {
		  ...
		  pCell->mulrk_.XFRK_[i].RKValue_ =
		      GetRKValueFromInteger(cell->GetInteger());
		}
		--c;
	      } else {
		// Single cell
	    ...
	    case BasicExcelCell::DOUBLE: {
	      // Check whether it is a single cell or range of cells.
	      // Double values which cannot be stored as RK values will be
	      // stored as single cells.
		...
		if (cellNext->Type() == BasicExcelCell::UNDEFINED ||
		    cellNext->Type() != cell->Type() ||
		    canStoreAsRKValue !=
			CanStoreAsRKValue(cellNext->GetDouble()))
		  break;
	      }
		...
		for (size_t i = 0; c < cl; ++c, ++i) {
		  cell = yesheets_[s].Cell(r, c);
		  pCell->mulrk_.XFRK_[i].RKValue_ =
		      GetRKValueFromDouble(cell->GetDouble());
		}
		--c;
	      } else {
		// Single cell
	    ...
	    case BasicExcelCell::STRING: {
	      ...
	      cell->Get(&*(str.begin()));
	      str.pop_back();  // Remove null character because LargeString does
			       // not store null character.
  ...
  for (size_t r = 0; r < maxRows_; ++r) {
	...
	case BasicExcelCell::STRING: {
	  if (textQualifier != '\0') {
	    ...
	    size_t npos = 0;
	    while ((it = find(cellString.begin() + npos, cellString.end(),
			      textQualifier)) != cellString.end()) {
	      npos = distance(cellString.begin(),
			      cellString.insert(it, textQualifier)) +
		     2;
	    }
...
size_t BasicExcelWorksheet::GetTotalRows() { return maxRows_; }

// Total number of columns in current Excel worksheet.
size_t BasicExcelWorksheet::GetTotalCols() { return maxCols_; }
...
BasicExcelCell* BasicExcelWorksheet::Cell(size_t row, size_t col) {
  // Check to ensure row and col does not exceed maximum allowable range for an
  // Excel worksheet.
...
void BasicExcelWorksheet::UpdateCells() {
  ...
  Worksheet::Dimensions& dimension =
      excel_->worksheets_[sheetIndex_].dimensions_;
  vector<Worksheet::CellTable::RowBlock>& rRowBlocks =
      excel_->worksheets_[sheetIndex_].cellTable_.rowBlocks_;
  ...
  for (size_t i = 0; i < maxRowBlocks; ++i) {
    vector<Worksheet::CellTable::RowBlock::CellBlock>& rCellBlocks =
	rRowBlocks[i].cellBlocks_;
	...
	case CODE::MULRK: {
	  size_t maxCols = rCellBlocks[j].mulrk_.lastColIndex_ -
			   rCellBlocks[j].mulrk_.firstColIndex_ + 1;
...
void BasicExcelCell::Set(int val) { SetInteger(val); }

// Set content of current Excel cell to a double.
void BasicExcelCell::Set(double val) { SetDouble(val); }

// Set content of current Excel cell to an ANSI string.
void BasicExcelCell::Set(const char* str) { SetString(str); }

// Set content of current Excel cell to an Unicode string.
void BasicExcelCell::Set(const wchar_t* str) { SetWString(str); }
